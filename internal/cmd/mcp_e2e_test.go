...
func connectE2E(t *testing.T, profile string) *mcp.ClientSession {
	t.Helper()
...
func TestE2E_ListTools(t *testing.T) {
	...
	for _, want := range []string{"version", "mail_list", "mail_delete"} {
		if !names[want] {
