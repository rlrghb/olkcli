...
func (c *ContactsListCmd) Run(ctx *RunContext) error {
	...
	headers := []string{"ID", "NAME", "EMAIL", "COMPANY", "TITLE"}
	var rows [][]string
	for _, ct := range contacts {
		...
		rows = append(rows, []string{id, ct.DisplayName, email, ct.Company, ct.JobTitle})
	}
...
func (c *ContactsGetCmd) Run(ctx *RunContext) error {
	...
	if len(contact.Phones) > 0 {
		fmt.Printf("Phone:   %s\n", outfmt.Sanitize(strings.Join(contact.Phones, ", ")))
	}
	if contact.Company != "" {
...
func (c *ContactsCreateCmd) Run(ctx *RunContext) error {
	...
	contact, err := client.CreateContact(ctx.Ctx, c.FirstName, c.LastName, c.Email, c.Phone, c.Company, c.Title)
	if err != nil {
		return err
	}
...
func (c *ContactsUpdateCmd) Run(ctx *RunContext) error {
	...
	var firstName, lastName, email, phone, company, title *string
	if c.FirstName != "" {
	...
	contact, err := client.UpdateContact(ctx.Ctx, c.ID, firstName, lastName, email, phone, company, title)
	if err != nil {
		return err
	}
...
func (c *ContactsSearchCmd) Run(ctx *RunContext) error {
	...
	headers := []string{"ID", "NAME", "EMAIL", "COMPANY"}
	var rows [][]string
	for _, ct := range contacts {
		id := outfmt.Truncate(ct.ID, 15)
		email := ""
		...
		rows = append(rows, []string{id, ct.DisplayName, email, ct.Company})
	}
